...
class ImageSearchRequest(BaseModel):
    ...
    image_url: Optional[str] = Field(
        ...
        example="https://example.com/fashion/dress.jpg"
    )
    ...
    exclude_reference: bool = Field(
        default=True,
        description="Whether to exclude the reference image from results"
    )
...
class FashionProduct(BaseModel):
    ...
    article_type: Optional[str] = Field(None, description="Type of article (e.g., Shirt, Dress)")
    master_category: Optional[str] = Field(None, description="Main category")
    ...
    season: Optional[str] = Field(None, description="Seasonal collection")
    year: Optional[int] = Field(None, description="Year of release")
    usage: Optional[str] = Field(None, description="Usage context (e.g., Casual, Formal)")
...
class SearchResponse(BaseModel):
    ...
    total_results: Optional[int] = Field(None, description="Total available results")
    results: List[FashionProduct] = Field(..., description="Search results")
    
    # Performance metrics
    performance: Dict[str, float] = Field(
        ...,
        description="Performance metrics in seconds"
    )
    
    # Search quality metrics
    similarity_stats: Optional[Dict[str, float]] = Field(
        None,
        description="Statistical summary of similarity scores"
    )
    ...
    image_source: Optional[str] = Field(None, description="Source of the image for image searches")
...
async def generate_text_embedding(text: str) -> List[float]:
    ...
    """
    try:
        inputs = resources.clip_processor(
            text=[text],
            return_tensors="pt",
            padding=True
        )
...
def format_search_results(
    ...
    for doc in results_to_process:
        formatted_results.append(FashionProduct(
            id=str(doc["_id"]),
...
async def health_check():
    ...
    try:
        ...
        components["mongodb"] = {
            ...
            "coverage_percentage": round((embedding_count / doc_count * 100) if doc_count > 0 else 0, 2)
        }
    except Exception as e:
        components["mongodb"] = {
            "status": "unhealthy",
    ...
    try:
        ...
        if components.get("clip_model", {}).get("status") == "healthy":
            ...
            test_results = await perform_vector_search(
    ...
    try:
        ...
        if request.exclude_reference and raw_results:
            ...
            filtered_results = [
                ...
                if r.get("similarity_score", 0) < 0.99
            ]
...
async def search_by_image_v2(
    ...
    try:
        ...
        if parsed_request.exclude_reference and raw_results:
            ...
            formatted_results = format_search_results(filtered_results, parsed_request.number_of_results)
        else:
    ...
    try:
        ...
        return {
            "database_stats": {
                ...
                "embedding_coverage": round((products_with_embeddings / total_products * 100) if total_products > 0 else 0, 2),
            ...
            "data_distribution": {
                "top_categories": [
                    ...
                    for cat in sample_categories if cat["_id"]
                ],
                "gender_distribution": [
                    {"gender": g["_id"], "count": g["count"]} 
                    for g in gender_dist if g["_id"]
                ]
            }
        }
